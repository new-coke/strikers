...
static void port_vid_note(const char* what, const SYNTH_VOICE* sv) {
  static int reported = 0;
  if (reported++ < 8) {
    if (sv != NULL) {
      fprintf(stderr, "[musyx] %s: voice id %08x parent %08x child %08x\n", what,
              (unsigned)sv->id, (unsigned)sv->parent, (unsigned)sv->child);
    } else {
      fprintf(stderr, "[musyx] %s\n", what);
    }
  }
}

static void vidRemove(VID_LIST** vidList) {
  /* PORT: nothing to unlink. */
  if (*vidList == NULL) {
    port_vid_note("vidRemove with no entry", NULL);
    return;
  }
...
void vidRemoveVoiceReferences(SYNTH_VOICE* svoice) {
  ...
  /* PORT: a live voice that holds no vid entry, every branch below either dereferences one or hands one to vidRemove. */
  if (svoice->vidList == NULL) {
    port_vid_note("live voice with no vid entry", svoice);
...
/* PORT: void, no path returned a value. */
static void voiceAllocateDo(s32 voice, u8 fxFlag) {
  s32 i;                // r30
...
u32 voiceAllocate(u8 priority, u8 maxVoices,
                  ...
                  u8 fxFlag) {
#if MUSY_VERSION >= MUSY_VERSION_CHECK(2, 0, 1)
  /* PORT: the console returned this through a register it happened not to clobber; there was no return statement. */
  {
    s32 voice = voiceAllocateFind(priority, maxVoices, allocId, fxFlag);
    voiceAllocateDo(voice, fxFlag);
    return (u32)voice;
  }
#else
  s32 i;                // r31
