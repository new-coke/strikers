...
#if MUSY_VERSION <= MUSY_VERSION_CHECK(2, 0, 1)
    /* PORT: uintptr_t, not s32, base is a host pointer, and on this target it stays one: hwSaveSample is a no-op off Dolphin. */
...
MSTEP* dataGetMacro(u16 mid) {
  ...
  if (dataMacMainTab[main].num != 0) {
    base = dataMacMainTab[main].subTabIndex;
    key.id = mid;
    /* PORT: sizeof, not 8, the record holds a pointer. */
