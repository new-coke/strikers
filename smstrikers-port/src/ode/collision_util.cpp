...
#define SET3(a, b, op, c) \
    ...
    if (da1 <= 0 && db1 >= 0)
    {
        *(volatile dReal*)&cp1[0] = a1[0];
        cp1[1] = *(volatile dReal*)&a1[1];
        cp1[2] = *(volatile dReal*)&a1[2];
        cp2[0] = *(volatile dReal*)&b1[0];
        cp2[1] = *(volatile dReal*)&b1[1];
        cp2[2] = *(volatile dReal*)&b1[2];
        return;
    }
