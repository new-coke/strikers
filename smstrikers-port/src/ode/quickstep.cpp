    ...
    dxQuickStepParameters* qs)
{
    ...
    IndexError* order = (IndexError*)__builtin_alloca(m * sizeof(IndexError));
    ...
    dxJoint* const* _joint, int nj, dReal stepsize)
{
    ...
    dxJoint** joint = (dxJoint**)__builtin_alloca(nj * sizeof(dxJoint*));
    ...
    for (i = 0; i < nb; i++)
    {
        ...
        tacc[2] -= tmp[1] * avel[0] - tmp[0] * avel[1];
    }
    ...
    dxJoint::Info1* info = (dxJoint::Info1*)__builtin_alloca(nj * sizeof(dxJoint::Info1));
    for (i = 0, j = 0; j < nj; j++)
    { // i=dest, j=src
    ...
    int m = 0;
    int* ofs = (int*)__builtin_alloca(nj * sizeof(int));
    for (i = 0; i < nj; i++)
    {
        ofs[i] = m;
        m += info[i].m;
    ...
    int* jb = (int*)__builtin_alloca(m * 2 * sizeof(int));
    if (m > 0)
    {
        ...
        int* findex = (int*)__builtin_alloca(m * sizeof(int));
        dSetZero(c, m);
    ...
    IFTIMING(dTimerNow("compute velocity update");)
    {
        ...
        {
            dxBody* b = *body_ptr;
            dReal step_invMass = stepsize * b->invMass;
            for (j = 0; j < 3; j++)
                b->lvel[j] += step_invMass * b->facc[j];
            for (j = 0; j < 3; j++)
                b->tacc[j] *= stepsize;
