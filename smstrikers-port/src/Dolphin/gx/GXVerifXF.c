...
static void CheckLightRAMRanges(void)
{
    ...
    for (lightSource = 0; lightSource < 8; lightSource++)
    {
        for (i = 1; i < 13; i++)
        {
            ...
            CheckFloatingPointValue(__gxVerif->xfLightDirty[lightRAMOffset - 0x600], __gxVerif->xfLight[(s32)(lightRAMOffset - 0x600)],
                                    label);
        }
    }
...
{
    if (!__GXVertexPacketHas(GX_VA_PNMTXIDX) || !__GXVertexPacketHas(GX_VA_TEX0MTXIDX) || !__GXVertexPacketHas(GX_VA_TEX1MTXIDX)
    ...
    if ((!__GXVertexPacketHas(GX_VA_TEX4MTXIDX) || !__GXVertexPacketHas(GX_VA_TEX5MTXIDX) || !__GXVertexPacketHas(GX_VA_TEX6MTXIDX)
...
static void CheckErrors(void)
{
    ...
    if (__gxVerif->verifyLevel >= 1)
    {
        ...
        if (numColorTextures != 0 && (u32)((BYTE3(__gxVerif->xfRegs[numRegularTextures + numBumpmapTextures + 64]) >> 4) & 7) != 2
