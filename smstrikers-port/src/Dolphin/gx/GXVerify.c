...
char* __gxvWarnings[125]
    = { "Invalid Vertex Format. Normal count must be set to %s.",
        ...
        "TexCoordGen types are out of order. GX_TG_MTX2x4/3x4 should come first (if any), followed by GX_TG_BUMP (if any), then GX_TG_SRTG "
        "(if any).",
        "Bumpmap texgen is defined, which requires that binormals and tangents be transformed by a normal matrix, but current matrix index "
        "is set to an invalid value (%d) for normal transform.",
        ...
        "Channel %d performs lighting which requires the normal to be transformed by a normal matrix, but current matrix index is (%d), "
        ...
        "Bumpmap texgen is defined, which requires the normal matrix values pointed by current matrix index (%d) to be loaded, however it "
        ...
        "Channel %d performs lighting which requires the normal matrix values pointed to by the current matrix index (%d), however these "
        "values may not be loaded yet.",
        ...
        "Output XF colors or color textures enabled, COLOR%dCNTRL_REG.material_src == REGISTER, but Material %d register (0x%04x) is not "
        "initialized.",
        "Output XF colors or color textures enabled, COLOR%dCNTRL_REG.ambient_src == REGISTER, but Ambient %d register (0x%04x) is not "
        "initialized." };
