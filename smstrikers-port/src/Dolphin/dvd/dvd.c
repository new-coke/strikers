...
static int issueCommand(s32 prio, DVDCommandBlock* block)
{
    ...
    if (autoInvalidation != 0
        && (block->command == DVD_COMMAND_READ || block->command == DVD_COMMAND_BSREAD || block->command == DVD_COMMAND_READID
...
s32 DVDCancel(volatile DVDCommandBlock* block)
{
    ...
    while (1)
    {
        ...
        if (state == DVD_STATE_COVER_CLOSED)
        {
            ...
            if ((command == DVD_COMMAND_BSREAD) || (command == DVD_COMMAND_READID) || (command == DVD_COMMAND_AUDIO_BUFFER_CONFIG)
